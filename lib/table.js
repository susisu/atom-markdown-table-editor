...
export class Table {
  ...
  format(options) {
    ...
    const minContentWidth = options.minContentWidth || 3;
    const useEAW = options.useEAW === undefined
      ? false
      : options.useEAW;
    ...
    const eawOptions = { useEAW, ambiguousAsWide };
    ...
    rows.push(new TableRow(
      this.rows[0].cells.map((cell, j) =>
        new TableCell(pad(
          align(cell.content, Alignment.CENTER, contentWidths[j], eawOptions)
        ))
      ),
      marginLeft,
      ''
    ...
    for (let i = alignmentRow ? 2 : 1; i < height; i++) {
      rows.push(new TableRow(
        this.rows[i].cells.map((cell, j) =>
          new TableCell(pad(
            align(cell.content, alignments[j], contentWidths[j], eawOptions)
          ))
        ),
        marginLeft,
        ''
...
function length(str, eawOptions) {
  if (eawOptions.useEAW) {
    let len = 0;
    for (const char of str) {
      // treat surrogate pairs as wide (not correct but optimal)
      if (char.length >= 2) {
        len += 2;
        continue;
      }
      switch (eaw.eastAsianWidth(char)) {
      case 'F':
      case 'W':
        len += 2;
        break;

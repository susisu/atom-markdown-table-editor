...
export class Table {
  ...
  format(options) {
    ...
    const defaultAlignment = options.defaultAlignment || DefaultAlignment.LEFT;
    const headerAlignment  = options.headerAlignment || HeaderAlignment.FOLLOW;
    const minContentWidth  = options.minContentWidth || 3;
    ...
    rows.push(new TableRow(
      this.rows[0].cells.map((cell, j) =>
        new TableCell(pad(align(
          cell.content,
          headerAlignment === HeaderAlignment.FOLLOW ? alignments[j] : headerAlignment,
          defaultAlignment,
          contentWidths[j],
          eawOptions
        )))
  ...
  formatEachRow(options) {
    ...
    rows.push(new TableRow(
      this.rows[0].cells.map((cell, j) =>
        new TableCell(pad(cell.content))
      ),
      marginLeft,
      ''
    ));
    ...
    if (alignmentRow) {
      ...
      rows.push(new TableRow(
        alignmentRow.cells.map((cell, j) =>
          new TableCell(alignmentText(alignments[j], minContentWidth))
        ),
        marginLeft,
        ''
      ));
...
function align(content, alignment, defaultAlignment, contentWidth, eawOptions) {
  const space = contentWidth - length(content, eawOptions);
  if (space < 0) {
    return content;
  }
  switch (alignment === Alignment.DEFAULT ? defaultAlignment : alignment) {
  case Alignment.RIGHT:

...
export class Table {
  ...
  complete(options) {
    ...
    const minAlignmentWidth = options.minAlignmentWidth || 3;
    ...
    if (alignmentRow) {
        ...
        extendArray(alignmentRow.cells, width, j =>
          new TableCell(alignmentText(
            ...
            j === alignmentRow.cells.length
              ? Math.max(minAlignmentWidth, alignmentRow.marginRight.length - 2)
              : minAlignmentWidth
          ))
        ),
      ...
      rows.push(new TableRow(
        extendArray([], width, () =>
          new TableCell(alignmentText(Alignment.DEFAULT, minAlignmentWidth))
        ),
        this.rows[0].marginLeft,
        ''
      ));

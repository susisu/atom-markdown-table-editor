...
export default class EditorController {
  ...
  isActive() {
    return !this.editor.hasMultipleCursors()
  ...
  getOptions() {
    ...
    return options({
      leftMarginChars  : new Set(atom.config.get(`${NAMESPACE}.leftMarginChars`, { scope })),
      formatType       : atom.config.get(`${NAMESPACE}.formatType`, { scope }),
      minDelimiterWidth: atom.config.get(`${NAMESPACE}.minDelimiterWidth`, { scope }),
      defaultAlignment : atom.config.get(`${NAMESPACE}.defaultAlignment`, { scope }),
      headerAlignment  : atom.config.get(`${NAMESPACE}.headerAlignment`, { scope }),
      smartCursor      : atom.config.get(`${NAMESPACE}.smartCursor`, { scope }),
      textWidthOptions : {
        normalize      : atom.config.get(`${NAMESPACE}.normalize`, { scope }),
        wideChars      : new Set(atom.config.get(`${NAMESPACE}.wideChars`, { scope })),
        narrowChars    : new Set(atom.config.get(`${NAMESPACE}.narrowChars`, { scope })),
        ambiguousAsWide: atom.config.get(`${NAMESPACE}.ambiguousAsWide`, { scope })
      }
    });
  }

...
export default class EditorController {
  constructor(editor) {
    this.editor = editor;
    this.scopes = atom.config.get(`${NAMESPACE}.scopes`);
  ...
  isInActiveScope() {
    ...
    const currentScopes = this.editor.scopeDescriptorForBufferPosition(pos).getScopesArray();
    for (let i = 0; i < this.scopes.length; i++) {
      if (currentScopes.indexOf(this.scopes[i]) >= 0) {
        return true;
      }
    }
    return false;
  ...
  isActive() {
    return !this.editor.hasMultipleCursors() && this.isInActiveScope() && this.isInTable();
  }
  ...
  getOptions() {
    const defaultAlignment  = atom.config.get(`${NAMESPACE}.defaultAlignment`);
    const headerAlignment   = atom.config.get(`${NAMESPACE}.headerAlignment`);
  ...
  format() {
    ...
    // format table
    const formatted = formatType === 'row'
      ? completed.table.formatEachRow(options)
      : completed.table.format(options);
  ...
  align(alignment) {
    ...
    let completedFocusCell = completed.table.getCell(newFocus.pos);
    if (completedFocusCell) {
      // copy cell since the content can be changed by alignment
      completedFocusCell = completedFocusCell.copy();
      // set alignment
  ...
  moveRow(direction) {
    ...
    // move row
    if (newFocus.pos.row >= 2) {
      if (direction > 0 && newFocus.pos.row < completed.table.height - 1) {
        const row = completed.table.rows[newFocus.pos.row];
        completed.table.rows.splice(newFocus.pos.row, 1);
        completed.table.rows.splice(newFocus.pos.row + 1, 0, row);
        newFocus.pos.row += 1;
      }
      else if (direction < 0 && newFocus.pos.row > 2) {
        const row = completed.table.rows[newFocus.pos.row];
        completed.table.rows.splice(newFocus.pos.row, 1);
        completed.table.rows.splice(newFocus.pos.row - 1, 0, row);
        newFocus.pos.row -= 1;
  ...
  moveColumn(direction) {
    ...
    // move column
    if (0 <= newFocus.pos.column && newFocus.pos.column < completed.table.headerWidth) {
      if (direction > 0 && newFocus.pos.column < completed.table.headerWidth - 1) {
        const column = completed.table.rows.map(row => row.cells[newFocus.pos.column]);
        for (let i = 0; i < completed.table.height; i++) {
          completed.table.rows[i].cells.splice(newFocus.pos.column, 1);
          completed.table.rows[i].cells.splice(newFocus.pos.column + 1, 0, column[i]);
        }
        newFocus.pos.column += 1;
      }
      else if (direction < 0 && newFocus.pos.column > 0) {
        const column = completed.table.rows.map(row => row.cells[newFocus.pos.column]);
        for (let i = 0; i < completed.table.height; i++) {
          completed.table.rows[i].cells.splice(newFocus.pos.column, 1);
          completed.table.rows[i].cells.splice(newFocus.pos.column - 1, 0, column[i]);
